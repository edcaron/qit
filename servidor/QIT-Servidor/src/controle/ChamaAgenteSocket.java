...
public class ChamaAgenteSocket {
    ...
    public void executarScriptNoCliente(MaquinasExecutarScript mes, Maquina maquina, Script script) {
        new Thread() {
            @Override
            public void run() {
                this.setName("executarScriptNoCliente " + maquina.getHost() + " " + Util.getCurrentTimestamp());
...
//        pega dados do agente
    ...
    public void executarInventarioNoCliente(Maquina maquina) {
        new Thread() {
            @Override
            public void run() {
                this.setName("executarInventarioNoCliente " + maquina.getHost()+ " " + Util.getCurrentTimestamp());
